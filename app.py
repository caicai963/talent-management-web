...
def gongzhang_yiti(demand_id):
    """标记公账已提：设置status=done, gongzhang_yiti=1，并发送入选通知"""
    conn = get_db()
    cursor = conn.cursor()
    ...
    if selected_list:
        ...
        if 'error' in result:
            return jsonify({'message': '已标记公账已提，但发送入选通知失败', 'gongzhang_yiti': True, 'selected_count': len(selected_list)})
...
def select_talent(app_id):
    ...
    demand_title = demand_row['title']; tidanren = demand_row.get('tidanren', ''); product_code = demand_row.get('product_code', '') if demand_row else '未知需求'
